...
metadata {
    ...
    command "cycleChild"	//can be called by smartApps to cycle through switches one at a time
...
def cycleChild() {			//called by smartApps to cycle through switches one at a time
...
def createChildVB(newTotal,vDetails, vType) {			//creates child Momentary switches
    ...
    for(i in 1..newTotal){
        ...
        def childDevice = getChildDevices()?.find {it.componentName == "VS-${i}"}
        if (!childDevice) {
           	log.info "Creating VS ${i}:${vName}"
        	childDevice = addChildDevice("Virtual $vType", "VS_${parent.app.id} ${i}", null,[completedSetup: true,
